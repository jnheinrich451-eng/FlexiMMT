...
from models.my_CogVideoI2V_train import CogVideoXTransformer3DModelIP
...
from models.my_pipeline_i2v_train import CogVideoXImageToVideoPipeline
from models.FlexiMMT_processor_mask_train import RefNetLoRAProcessor
import itertools
...
def main(args):
    ...
    trainable_key = 'inversion_tokens'if args.stage == 0 else 'lora'
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                with torch.no_grad():
                    ...
                    # Find concept and motion word positions
                    motion_indices = batch["motion_indices_list"][0]
                    ...
                    # boost training
                    # alpha = 3.0
                    # uniform_samples = torch.rand(b, device=image_latents.device)
                    # timesteps = (scheduler.config.num_train_timesteps * (1 - uniform_samples ** alpha))
                    # timesteps = torch.clamp(timesteps, 0, scheduler.config.num_train_timesteps - 1)
            ...
            # Evaluation
            eval_step = 0
            if (global_step % args.validating_steps == 0 and global_step > eval_step) \
                ...
                if args.stage is not None and args.val_csv_path is not None:
                    ...
                    for idx, row in val_df.iterrows():
                        ...
                        motion_words = row['motion_words']
                        motion_indices = get_motion_token_indices(tokenizer, prompt, [concept_words, motion_words])

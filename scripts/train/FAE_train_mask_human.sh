...
main_process_port=$((20118 + visible_devices))
...
CUDA_VISIBLE_DEVICES=$visible_devices accelerate launch --config_file $config_file --machine_rank 0 --num_processes $num_processes --main_process_port $main_process_port \
  ...
  --meta_file_path benchmark_new/captions_train/human/${motion_type}/crop.csv \
  --val_csv_path benchmark_new/captions_train/human/${motion_type}/val_image.csv \
